...
def csvdir_equities(tframes=None, csvdir=None, exchange=None, has_fundamentals=False):
	"""
	...
	return CSVDIRBundle(tframes, csvdir, exchange,has_fundamentals).ingest
...
class CSVDIRBundle:
	...
	def __init__(self, tframes, csvdir, exchange, has_fundamentals):
	...
	def _pricing_iter(self, csvdir, symbols, metadata, divs_splits, show_progress, fundamentals_writer=None):
		with tqdm(symbols, desc='Loading custom pricing data: ') as it:
			...
			for sid, symbol in enumerate(it):
				it.set_description('Symbol {} sid {}'.format(symbol, sid))
				try:
					fname = [fname for fname in files
							 if '{}.csv'.format(symbol) in fname][0]
				except IndexError:
					raise ValueError("{}.csv file is not in {}".format(symbol, csvdir))
				...
				if fundamentals_writer is not None:
					fcsvpath = os.path.join(csvdir, 'fundamentals', '%s.csv' % symbol)
...
# 							dividends=divs_splits['divs'])
# 	fundamentals_writer.write(fundamentals=DataFrame())
#
#
# def _pricing_iter(csvdir, symbols, metadata, fundamentals,divs_splits, show_progress):
# 	with tqdm(symbols, desc ='Loading custom pricing data: ') as it:
# 		files = os.listdir(csvdir)
# 		for sid, symbol in enumerate(it):
...
# 			fcsvpath = os.path.join(csvdir, 'fundamentals', '%s.csv' % symbol)
# 			if os.path.isfile(fcsvpath):
# 				funds = read_csv(fcsvpath, parse_dates=[1],											                                           infer_datetime_format = True)
# 				funds['sid'] = sid
# 				fundamentals = fundamentals.append(funds)
#
# 			yield sid, dfr
