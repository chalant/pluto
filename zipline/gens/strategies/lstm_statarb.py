...
def before_trading_start(context, data):
	...
	api.schedule_function(rebalance, date_rules.week_start(days_offset=0), time_rules.market_open(hours=1, minutes=30))
	...
	# and filters.
	context.output = api.pipeline_output('mean_reversion_example')
...
def rebalance(context, data):
	...
	for stock in context.security_list:
		if data.can_trade(stock):
			if stock in context.long_secs.index:
				api.order_target_percent(stock, long_weight)
			elif stock in context.short_secs.index:
				api.order_target_percent(stock, short_weight)
	...
	for stock in context.portfolio.positions:
		if stock not in context.security_set and data.can_trade(stock):
			api.order_target_percent(stock, 0)

	# Log the long and short orders each week.
	log.info("This day's longs: " + ", ".join([long_.symbol for long_ in context.long_secs.index]))
	log.info("This day's shorts: " + ", ".join([short_.symbol for short_ in context.short_secs.index]))
