...
from contrib.data.traffic.download.executors_factory import order_executor
from contrib.data.traffic.download.executors import Schedule
from contrib.data.traffic.download.dispatchers import Dispatcher
...
# TODO: put all the api_keys in the environment... not in the module...
...
class Downloader(ABC):
	...
	def download(self, thread_pool, today=None, show_progress=False):
		if not today:
...
class DailyEquityDownloader(GroupDownloader):
	...
	def _download_by_asset_group(self, thread_pool, today, requests, asset_group):
		...
		#TODO: change this method: the method only gets today and doesn't handle
		#the calendar...
...
class EquityGroup(ABC):
	def __init__(self, assets_collection):
		self._asset_finder = assets_collection

	def __iter__(self):
		for asset in self._create_generator(self._asset_finder):
			exc = asset.exchange
