...
class DailyLedgerField(Metric):
    ...
    def end_of_bar(self, **kwargs):
        ...
        packet['cumulative_perf'][field] = packet['minute_perf'][field] = (self._get_ledger_field(kwargs.pop('ledger')))

    def end_of_session(self, **kwargs):
        field = self._packet_field
        packet = kwargs['ledger']
        packet['cumulative_perf'][field] = packet['daily_perf'][field] = (self._get_ledger_field(kwargs.pop('ledger')))
...
class Returns(Metric):
    def _end_of_period(self, field, packet, ledger):
        ...
        packet['cumulative_risk_metrics']['algorithm_period_returns'] = (ledger.portfolio.returns)
...
class BenchmarkReturnsAndVolatility(Metric):
    # todo: should optimize this class since we are recomputing without caching
    def initialization(self, **kwargs):
        self._first_open = kwargs.pop('first_open_session')
    ...
    def end_of_bar(self, **kwargs):
        ...
        if emission_rate == 'minute':
            ...
            returns = benchmark_source.get_range(self._first_open, self._last_close)
    ...
    def end_of_session(self, **kwargs):
        ...
        self._last_close = kwargs.pop('dt')
